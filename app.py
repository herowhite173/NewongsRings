...
def main() -> None:
    ...
    st.markdown(
        ...
        @media only screen and (max-width: 768px) and (orientation: portrait) {
            .stMarkdown h3 {display: none !important;}
            .stMarkdown h4 {
    ...
    # ============ 拆分手机 / PC ============
    if mobile_view:
        # 手机端：从上到下：按钮组→移动按钮→图片→导出/数据（你已经改好的结构不动）
        # 顶部原并排按钮保留
        ...
        if demo_enabled:
            with col_left_move:
                if st.button("向左移动", use_container_width=True, key="left_move_btn_m"):
                    ...
                    if step > 0:
                        ...
                        st.rerun()
            with col_right_move:
                if st.button("向右移动", use_container_width=True, key="right_move_btn_m"):
            ...
            with col_left_move:
                if st.button("左34级条纹", use_container_width=True, key="left_34_btn_m"):
                    R = st.session_state.R_mm * 1e-3
                    lam = 589.3e-9
                    st.session_state.total_shift = -np.sqrt(34 * lam * R)
                    st.rerun()
            with col_right_move:
                if st.button("向右移动2级", use_container_width=True, key="right_move_btn_sim_m"):
                    R = st.session_state.R_mm * 1e-3
                    lam = 589.3e-9
                    step = get_step_to_adjacent_fringe(direction='right')
                    if step > 0:
        ...
        if sim_enabled:
            ...
            st.dataframe(df, use_container_width=True)
            st.markdown("<br>")
            data_complete = all(v is not None for v in st.session_state.left_pos.values()) and all(v is not None for v in st.session_state.right_pos.values())
            if data_complete:
                st.button("📊 不确定度", type="primary", use_container_width=True, on_click=show_uncertainty_dialog)
            else:
                remaining = sum(1 for v in st.session_state.left_pos.values() if v is None) + sum(1 for v in st.session_state.right_pos.values() if v is None)
